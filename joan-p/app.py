...
# ── Pre-warm: load BD + prompt on first run (shows spinner, not blank screen) ─
@st.cache_resource(show_spinner="⏳ Cargando base de datos de kits… (solo la primera vez)")
def _warm_up():
    from selector import get_system_prompt
    get_system_prompt()   # builds & caches the slim CSV + prompt
    return True
